...
static const int combtuningR[numcombs] = { 1116 + stereospread, 1188 + stereospread,
...
static const int allpasstuningR[numallpasses] = { 556 + stereospread,
                                                  441 + stereospread,
                                                  341 + stereospread,
                                                  225 + stereospread };
...
static void dsp_add_freeverb(t_freeverb *x,
                             t_sample *in1,
                             t_sample *in2,
                             t_sample *out1,
                             t_sample *out2,
...
static void freeverb_mute(t_freeverb *x) {
  ...
  if (freeverb_getmode(x) >= freezemode) return;
...
void freeverb_tilde_setup(void) {
  ...
  class_addmethod(
      freeverb_class, (t_method)freeverb_setroomsize, gensym("roomsize"), A_FLOAT, A_NULL);
  ...
  class_addmethod(freeverb_class,
                  (t_method)freeverb_setfirstpassfilter,
                  gensym("firstpassfilter"),
                  A_FLOAT,
                  A_NULL);
